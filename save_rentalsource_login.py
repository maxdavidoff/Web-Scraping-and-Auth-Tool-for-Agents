...
from src.rentalsource_agent.config import ensure_dirs, get_settings


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Open RentalSource, let you log in manually, then save the browser session."
    )
    ...
    try:
        ...
        print("\nLog into RentalSource in the browser window if you need an authenticated session.")
        print("When you can see your account/search page, return here and press ENTER.")
        input("Press ENTER to save the browser session... ")
        save_storage_state(context, settings.state_file)
        print(f"Saved RentalSource session to: {settings.state_file}")
    finally:
        context.close()
        browser.close()
        playwright.stop()
